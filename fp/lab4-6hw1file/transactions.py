...
def mainMenu():
    ...
    while option != 0:
        ...
        if option == 1:
            ...
            menu1(transList, transListHistory)
        elif option == 3:
            menu2(transList, transListHistory)
        elif option == 4:
            menu3(transList, transListHistory)
        elif option == 5:
            menu4(transList, transListHistory)
        elif option == 6:
            menu5(transList, transListHistory)
        elif option == 7:
            transList = undo(transList, transListHistory)
        elif option != 0:
            print('Optiune gresita; incercati din nou')

def updateHistory(transList ,transListHistory):
    """
    functia adauga lista curenta de tranzactii in transListHistory
    transList: lista curenta de tranzactii
    transListHistory: istoria listelor de tranzactie
    """
    if (len(transListHistory) <= 1 and 
        transList != transListHistory[len(transListHistory) - 1]):
        transListHistory.append(transList)
        return
    if transList != transListHistory[len(transListHistory) - 1]:
        transListHistory.append(transList)
        
def undo(transList, transListHistory):
    """
    functia aduce transList (lista de tranzactii) la forma pe care aceasta
        o avea inainte de ultima operatie facuta asupra ei
    transList: lista de tranzactii curenta
    transListHistory: lista de liste de tranzactii anterioare
    """
    if len(transListHistory) > 1:
        transListHistory.pop()
        transList = transListHistory.pop()
        transListHistory.append(transList)
    else:
        transList = []
    return transList

def menu1(transList, transListHistory):
    """
    ...
    while option != 0:
        ...
        if option == 1:
            ...
            print('Optiune gresita; incercati din nou')
        updateHistory(transList, transListHistory)

def menu2(transList, transListHistory):
    """
...
def menu3(transList, transListHistory):
    """
        ...
        if option == 1:
            ...
            if not checkAmount(amount):
                print('suma invalida (suma trebuie sa fie o valoare',
                    'strict pozitiva)')
                continue
            printTransHigherThan(transList, amount)
        elif option == 2:
            day = int(input('ziua: '))
            if not checkDay(day):
                print('ziua invalida (ziua trebuie sa fie un numar intreg cu)',
            ...
            if not checkAmount(amount):
                print('suma invalida (suma trebuie sa fie o valoare',
                    'strict pozitiva)')
                continue
            printTransBeforeDayHigherThan(transList, day, amount)
        elif option == 3:
            typ = input('tipul: ')
            if not checkType(typ):
                print("tip invalid (tipul poate fi doar 'intrare' si 'iesire')")
                continue
            printTransOfType(transList, typ)
        elif option != 0:
            print('Optiune gresita; incercati din nou')
        updateHistory(transList, transListHistory)

def menu4(transList, transListHistory):
    """
    meniu de rapoarte
    """
    option = 1
...
def menu5(transList, transListHistory):
    """
    meniu filtrare
    """
    option = 1
